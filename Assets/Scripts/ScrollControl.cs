...
public class ScrollControl : MonoBehaviour
{
    ...
    public void ScrollController()
    {
        if (scrollRect.verticalNormalizedPosition <= verticalNormalizedDownThresholdValue)
        {
            ...
            ChangeContentChildPos(leftBoxId, rightBoxId, -1110);

            if (leftBoxId == contenChildtList.Count - 2)
            {
                leftBoxId = 0;
                rightBoxId = 1;
                verticalNormalizedDownThresholdValue -= 2f;
            }
            else
            {
                leftBoxId += 2;
            ...
            verticalNormalizedUpThresholdValue = verticalNormalizedDownThresholdValue + 1f;
        }
        else if (scrollRect.verticalNormalizedPosition > verticalNormalizedUpThresholdValue)
        {
            ...
            if (leftBoxId == 0)
            {
                leftBoxId = 10;
                rightBoxId = 11;
                verticalNormalizedUpThresholdValue += 2f;
            }
            else
            {
                leftBoxId -= 2;
            ...
            verticalNormalizedDownThresholdValue = verticalNormalizedUpThresholdValue - 1;
        }
    }
}